    ...
    case socket.assigns.state do
      :active ->
        if new_position == socket.assigns.selection do
          ...
          is_valid_selection =
            !GameBoard.is_empty?(socket.assigns.board, new_position) and
              elem(GameBoard.value(socket.assigns.board, new_position), 0) == socket.assigns.turn
            ...
            selection ->
              ...
              cond do
                Enum.member?(valid_moves, new_position) ->
                  assign(socket,
                    board: GameBoard.move(socket.assigns.board, selection, new_position),
                    selection: nil,
                    turn: next_turn(socket.assigns.turn)
                  )
